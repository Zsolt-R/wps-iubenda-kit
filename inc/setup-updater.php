...
namespace WPS\IubendaKit\Inc\Updater;
...
function plugin_info( $res, string $action, object $args ) { //phpcs:ignore
	...
	if ( WPS_IUBENDA_KIT_PLUGIN_SLUG !== $args->slug ) {
		return $res;
	}

	$remote = get_transient( 'wps_iubenda_kit_upgrade_' . WPS_IUBENDA_KIT_PLUGIN_SLUG );

	if ( false === $remote ) {

		$remote = wp_remote_get(
			WPS_IUBENDA_KIT_UPDATE_URL . '/get-info.php?slug=' . WPS_IUBENDA_KIT_PLUGIN_SLUG . '&action=info',
			[
				'timeout' => 5,
				'headers' => [
		...
		if ( ! is_wp_error( $remote ) && isset( $remote['response']['code'] ) && 200 === $remote['response']['code'] && ! empty( $remote['body'] ) ) {
			set_transient( 'wps_iubenda_kit_upgrade_' . WPS_IUBENDA_KIT_PLUGIN_SLUG, $remote, 21600 ); // 6 hours cache.
		}
	}
...
function plugin_push_update( $transient ) { //phpcs:ignore
	...
	if ( false === $remote || isset( $remote->error ) ) {
		...
		$remote = wp_remote_get( WPS_IUBENDA_KIT_UPDATE_URL . '/get-info.php?slug=' . WPS_IUBENDA_KIT_PLUGIN_SLUG . '&action=update',
			[
				'timeout' => 10,
				'headers' => [
					'Accept' => 'application/json',
	...
	if ( $remote && ! isset( $remote->error ) ) {
		...
		if ( $remote && version_compare( WPS_IUBENDA_KIT_VERSION, $remote->version, '<' ) && version_compare( $remote->requires, get_bloginfo( 'version' ), '<' ) ) {
			$res       = new \stdClass();
			$res->slug = WPS_IUBENDA_KIT_PLUGIN_SLUG;

			// it could be just mypluginslug1.php if your plugin doesn't have its own directory (my does).
			$res->plugin                         = WPS_IUBENDA_KIT_PLUGIN_SLUG . '/' . WPS_IUBENDA_KIT_PLUGIN_SLUG . '.php';
...
function plugin_after_update( \WP_Upgrader $upgrader_object, array $options ): void {
	if ( 'update' === $options['action'] && 'plugin' === $options['type'] ) {
		...
		delete_transient( 'wps_iubenda_kit_upgrade_' . WPS_IUBENDA_KIT_PLUGIN_SLUG );
	}
}
